...
class MainWindow(QMainWindow):
    ...
    # Пароль для открытия окна с изменением меню
...
class MenuWidget(QMainWindow):
    ...
    # установка меню на сегодня (если сегодня вс - на пн)    
    def set_today_menu(self):
        day = datetime.datetime.now().isoweekday()
        if day == 7:
            day = 0
        self.title.setText(self.title.text()[:8] + WEEK[day - 1])
    ...
    # добавление списка блюд при смене дня недели в toolbar
    def set_buffet(self):
...
class AssessWidget(QMainWindow):
    ...
    # добавление в файл информации из анкеты после нажатия кнопки "отправить отзыв"
    def sending(self):
        try:
...
class InfoWidget(QMainWindow):
    ...
    # создание рейтинга блюд, используя информацию из файла, выведение ее в приложение
    def set_text(self):
        try:
            file = open('feedback.txt', 'r')
...
class ChangesWidget(QMainWindow):
    def __init__(self, *args):
        ...
        self.lunch.triggered.connect(self.make_the_dif)
        self.buffet.triggered.connect(self.make_the_dif)
        
    # установка таблицы, в которой будут происходить изменения
    def make_the_dif(self):
        if self.sender().text() == 'Буфет':
            self.title.setText(self.title.text()[:15] + 'буфета')
            self.table = 'buffet'
        else:
            self.title.setText(self.title.text()[:15] + 'столовой')
            self.table == 'menu'
            
    # изменеия в таблице буфета
    def change_buf(self):
        CUR.execute("""UPDATE buffet SET dish = ? WHERE dish = ?""", (self.new_name, self.name))
        CON.commit()
        CUR.execute("""UPDATE buffet SET weight = ? WHERE dish = ?""", (self.weight, self.new_name))
        CON.commit()
        CUR.execute("""UPDATE buffet SET calories = ? WHERE dish = ?""", (self.calories, self.new_name))
        CUR.execute("""UPDATE buffet SET poteins = ? WHERE dish = ?""", (self.proteins, self.new_name))
        CON.commit()
        CUR.execute("""UPDATE buffet SET fats = ? WHERE dish = ?""", (self.fats, self.new_name))
        CUR.execute("""UPDATE buffet SET carbohydrates = ? WHERE dish = ?""", (self.carbohydrates, self.new_name))
        CON.commit()
        self.confirm.setText('Все изменения подтверждены.')
        self.for_errors.setText('')

    # определение набора данных - меню буфета или обеды
    def set_table(self):
        if self.table == 'menu':
            self.data_base = [i[0] for i in CUR.execute("SELECT dish FROM menu").fetchall()]
            self.change()
        else:
            self.data_base = [i[0] for i in CUR.execute("SELECT dish FROM buffet").fetchall()]
            self.change()
    
    # изменение переменных, содержащих информацию о выбранном блюде
    def change(self):
        try:
    ...
    def change_dish(self):
        CUR.execute("""UPDATE menu SET dish = ? WHERE dish = ?""", (self.new_name, self.name))
        CON.commit()
        CUR.execute("""UPDATE menu SET weight = ? WHERE dish = ?""", (self.weight, self.new_name))
        CON.commit()
        CUR.execute("""UPDATE menu SET calories = ? WHERE dish = ?""", (self.calories, self.new_name))
        CUR.execute("""UPDATE menu SET proteins = ? WHERE dish = ?""", (self.proteins, self.new_name))
        CON.commit()
        CUR.execute("""UPDATE menu SET fat = ? WHERE dish = ?""", (self.fats, self.new_name))
        CUR.execute("""UPDATE menu SET carbohudrates = ? WHERE dish = ?""", (self.carbohydrates, self.new_name))
        CON.commit()
        self.confirm.setText('Все изменения подтверждены.')
        self.for_errors.setText('')
